...
Template.CustomersInsertInsertForm.events({
	"submit": function(e, t) {
		...
		validateForm(
			...
			function(values) {
				...
				alert(JSON.stringify(values, null, ' ')); newId = Customers.insert(values, function(e) { if(e) errorAction(e); else submitAction(); });
			}
		);
	...
	'input .happy-field': function(e, t) { var input = t.$(e.currentTarget); input.parent().find('label').text(input.val() || ':P'); }
});
